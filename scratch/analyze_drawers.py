...
m_ped = YOLO('computer_vision_models/yolo26/fine_tune_model/pediatric-model.pt')
tracker = sv.ByteTrack(track_activation_threshold=0.25, lost_track_buffer=60, minimum_matching_threshold=0.50)

track_data = {}

with VideoReader('assets/cctv_videos/cctv_child_room_drawers.mp4') as r:
    for idx, frame in r.frames():
        ...
        if tracked.tracker_id is not None:
            for i, tid in enumerate(tracked.tracker_id):
                ...
                track_data[tid]['h_ratios'].append(h / 270.0)

print('Track summaries for child room drawers:')
